...
def get_chat_response(user_message, chat_history=None, is_onboarding_done=False, user_animal=""):
    ...
    if not is_onboarding_done:
        ...
        2. 질문을 던질 때는 무조건 아래 '포맷 1'을 지켜. 
        3. 사용자의 대답에 대한 인사말, 리액션, 공감 멘트, 부연 설명은 절대 하지 마! 오직 제시된 문항표의 "1. 질문 내용" 리터럴 텍스트만 "title"에 정확하고 깔끔하게 넣어.
        4. 20번 문항까지 대답이 끝나면, 점수를 종합해 7가지 동물 중 하나를 고르고 '포맷 2' 형식으로만 출력하며 대화를 끝내.

        [출력 JSON 포맷 1: 질문할 때]
        {
          "type": "select",
          "title": "1. 아침에 눈을 떴을 때, 오늘 하루의 일정이 꽉 차 있다면?",
          "detail": [
    ...
    try:
        if not is_onboarding_done:
            ...
            return {"is_finished": True, "reply_message": response.choices[0].message.content.strip()}
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # 1. 20문항 끝나고 최종 결과가 나온 경우
        if bot_response.get("is_finished"):
            print("\n🎉 [테스트 완료! 프론트엔드로 넘어갈 최종 데이터]")
            print(json.dumps(bot_response, indent=2, ensure_ascii=False))
            break
            
        # 2. 테스트 진행 중 (프론트엔드 요구사항: 3지선다 객관식 JSON)
        elif bot_response.get("type") == "select":
            print("\n📦 [프론트엔드로 전달되는 실제 JSON 데이터 (그대로 복붙해서 프론트에게 보여주세요!)]")
            print(json.dumps(bot_response, indent=2, ensure_ascii=False))
            
            print("\n📱 [프론트엔드 화면 시뮬레이션]")
            print(f"🤖 챗봇 말풍선: {bot_response['title']}")
            print("👇 하단 버튼 3개:")
            for i, option in enumerate(bot_response['detail']):
                print(f"   [{i+1}] {option}")
            
            # 다음 대화를 위해 기록 업데이트 (시스템이 JSON 형태를 기억하도록 문자열로 변환하여 저장)
